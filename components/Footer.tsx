...
export function Footer() { return <footer className="border-t border-white/10 bg-[#090a0b]">
  <div className="public-container py-14 md:py-20"><div className="grid gap-12 md:grid-cols-2 lg:grid-cols-[1.4fr_.7fr_.7fr_.9fr]">
    <div><div className="flex items-center gap-3"><DraglineMark size={34}/><span className="font-display text-xl font-black">DRAGLINE<span className="text-amber">/</span>3D</span></div><p className="body-copy mt-5 max-w-sm">Custom additive manufacturing for functional parts, prototypes, replacement parts, and short production runs.</p><div className="mt-6 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[.15em] text-steel"><span className="status-dot"/>Louisville, Kentucky</div></div>
  ...
  </div><div className="mt-14 flex flex-col gap-3 border-t border-white/10 pt-6 font-mono text-[9px] uppercase tracking-[.15em] text-steel sm:flex-row sm:justify-between"><span>© {new Date().getFullYear()} Dragline 3D LLC</span><span>Functional parts. Layer by layer.</span></div></div>
  </footer>; }
