...
export async function POST(request: Request) {
  ...
  if (!shippoKey) return Response.json({ error: "Shippo not configured" }, { status: 503 });

  // Convert grams to oz (Shippo uses oz)
  ...
  try {
    ...
    if (!resp.ok) return Response.json({ error: "Shippo error" }, { status: 502 });
    ...
    const rates = (data.rates || [])
      ...
      .map((r: any) => ({
        id: r.object_id,
        provider: r.provider,
        service: r.servicelevel?.name || r.service_level?.name || r.service,
