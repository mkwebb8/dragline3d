...
export async function POST(request: Request) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:700px;margin:0 auto">
      ...
      <div style="border:1px solid #ddd;border-top:none;padding:24px;border-radius:0 0 4px 4px">
        ...
        <h2 style="color:#333;border-bottom:2px solid #ffb547;padding-bottom:8px;margin-top:24px">Total</h2>
        <table style="font-size:15px">
          <tr><td style="padding:4px 16px 4px 0">Parts:</td><td><strong>$${(parseFloat(total) - parseFloat(shippingCost)).toFixed(2)}</strong></td></tr>
          <tr><td style="padding:4px 16px 4px 0">Shipping (${shippingLabel}):</td><td><strong>$${parseFloat(shippingCost).toFixed(2)}</strong></td></tr>
          <tr style="font-size:18px"><td style="padding:8px 16px 4px 0"><strong>Total charged:</strong></td><td><strong>$${parseFloat(total).toFixed(2)}</strong></td></tr>
        </table>
        <p style="margin-top:24px;color:#666;font-size:13px">STL/3MF files attached. Reply to ${customerEmail} with questions.</p>
      </div>
    </div>
  `;
  ...
  for (const item of items) {
    ...
    if (file) {
      ...
      attachments.push({ filename: item.fileName, content: base64 });
    }
  }
  ...
  if (!resendResp.ok) {
    console.error("Resend error:", await resendResp.json());
