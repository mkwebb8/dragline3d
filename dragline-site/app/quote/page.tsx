...
export default function QuotePage() {
  ...
  async function handleFile(f: File | undefined) {
    ...
    if (!isSTL && !is3MF) {
      setError("STL or 3MF files only.");
      return;
    }
    setError(null);
    setFile(f);
    ...
    try {
      ...
      const geo = /\.3mf$/i.test(f.name)
        ? await (await import("@/lib/parse3mf")).parse3MF(buffer)
        : parseSTL(buffer);
      geo.computeBoundingBox();
      const size = new THREE.Vector3();
      geo.boundingBox!.getSize(size);
      const { computeVolume } = await import("@/lib/stl");
      const volumeMm3 = computeVolume(geo);
      setStats({ dims: { x: size.x, y: size.y, z: size.z }, volumeMm3 });
      setGeometry(geo);
    } catch {
      setError("Could not parse file — may be corrupt or unsupported.");
      setLoading(false);
    }
  }
    ...
    <div className="max-w-6xl mx-auto px-6 py-12 md:py-16">
      ...
      <div className="mb-12 max-w-3xl">
        ...
        <p className="text-bone/70 text-lg leading-relaxed max-w-xl">
          Real prices in seconds. STL and 3MF files accepted — drag and drop or click to browse.
