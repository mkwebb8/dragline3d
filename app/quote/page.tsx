...
import { parseSTL, computeVolume, quoteFromGeometry, computeSetupAndHandling, JOB_SETUP_FEE, HANDLING_FEE_PER_UNIT, MATERIALS, QUALITIES, MATERIAL_COLORS, type MaterialKey, type QualityKey } from "@/lib/stl";
...
export default function QuotePage() {
  ...
  useEffect(() => {
    try {
      ...
      const reorderFiles: Record<string, { base64: string; mimeType: string }> = reorderRaw ? JSON.parse(reorderRaw) : {};
      ...
      if (shippingRaw) {
        ...
        if (s.localPickup) { setLocalPickup(true); }
        else {
  ...
  // ── Cart-wide bulk pricing: parts subtotal is PURE material+machine cost now ──
  const partsSubtotal = cartItems.reduce((sum, i) => sum + i.quote.price * i.qty, 0);
  ...
  const cartSubtotal = partsSubtotal + setupAndHandling;
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      notifyForm.append("setupFee", String(setupFee));
      notifyForm.append("handlingFee", String(handlingFee));
      notifyForm.append("handlingUnits", String(handlingUnits));
      ...
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({
          ...
          setupFee, handlingFee, handlingUnits,
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-14">
        ...
        <div className="mb-14">
          ...
          <p className="text-bone/50 text-base leading-relaxed max-w-xl">
            Upload individual STL, 3MF, or STEP files — one file per part. Configure each part, add them to your cart, then checkout.
          </p>
        </div>
        ...
        <div className="grid xl:grid-cols-5 gap-5">
            ...
            {stats && (
              <div className="rounded-2xl p-6" style={glass}>
                ...
                <div className="mt-2 text-center font-mono text-[9px] text-steel/60">
                  Price shown is material + machine cost. Job setup (${JOB_SETUP_FEE}) & per-part handling (${HANDLING_FEE_PER_UNIT}) apply once per cart — see cart summary.
                </div>
              </div>
            )}
          </div>
          ...
          <div className="xl:col-span-2 space-y-4">
            <div className="rounded-2xl overflow-hidden" style={glass}>
              ...
              {cartItems.length === 0 ? (
                ...
                <div>
                  ...
                  {/* Setup & Handling — cart-wide line, recalculates live as parts/qty change */}
                  <div className="px-5 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,181,71,0.03)" }}>
                    ...
                    <div className="font-mono text-[9px] text-steel/60 mt-1.5">
                      Covers queueing, file tracking, part labeling, removal & packing — applied once per order.
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="rounded-2xl overflow-hidden" style={glass}>
              <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                <div className="font-display font-bold text-lg flex items-center gap-2">
                  <Truck size={17} style={{ color: "#ffb547" }} />
                  Shipping
                </div>
              </div>
              <div className="px-5 py-5 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <input value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="First name"
                    className="px-3 py-2.5 rounded-xl text-bone text-sm transition-all duration-150"
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <input value={city} onChange={e => setCity(e.target.value)} placeholder="City"
                        className="px-3 py-2.5 rounded-xl text-bone text-sm transition-all duration-150" style={inputBase} onFocus={focusOn} onBlur={focusOff} />
                      <input value={stateField} onChange={e => setStateField(e.target.value)} maxLength={2} placeholder="State"
                        className="px-3 py-2.5 rounded-xl text-bone text-sm transition-all duration-150" style={inputBase} onFocus={focusOn} onBlur={focusOff} />
                      ...
                      <div className="space-y-2">
                        {shippingRates.map(rate => (
                          <label key={rate.id} className="flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all duration-150"
                            ...
                            <div className="flex items-center gap-3">
                              <input type="radio" name="shipping" checked={selectedRateId === rate.id} onChange={() => setSelectedRateId(rate.id)} className="accent-amber" />
                              <div>
            ...
            {cartItems.length > 0 && (
              <div className="rounded-2xl p-6" style={{ background: "rgba(255,181,71,0.07)", border: "1px solid rgba(255,181,71,0.22)", boxShadow: "0 0 40px rgba(255,181,71,0.07)" }}>
                ...
                <div className="space-y-2 mb-5 font-mono text-sm">
                  <div className="flex justify-between"><span className="text-bone/50">Parts ({totalUnits} units)</span><span className="font-bold text-bone">${partsSubtotal.toFixed(2)}</span></div>
                  <div className="flex justify-between"><span className="text-bone/50">Setup & Handling</span><span className="font-bold text-bone">${setupAndHandling.toFixed(2)}</span></div>
                ...
                <button onClick={handleCheckout} disabled={checkingOut || (!selectedRate && !localPickup)}
                  ...
                  {checkingOut ? "REDIRECTING..." : <>PAY WITH SQUARE <ArrowRight size={16} /></>}
                </button>
