...
export async function PATCH(request:Request,{params}:{params:Promise<{plateId:string}>}){
  if(!await verifyAdminToken(request))return Response.json({error:"Unauthorized"},{status:401});
  const{plateId}=await params;
  const{status}=await request.json();
  const r=await supabase(`plates?id=eq.${plateId}`,{method:"PATCH",body:JSON.stringify({status,updated_at:new Date().toISOString()})});
  if(!r.ok)return Response.json({error:"Failed to update plate"},{status:500});
  const[plate]=await r.json();
  return Response.json(plate);
}

export async function DELETE(request:Request,{params}:{params:Promise<{plateId:string}>}){
  if(!await verifyAdminToken(request))return Response.json({error:"Unauthorized"},{status:401});
  const{plateId}=await params;
  const r=await supabase(`plates?id=eq.${plateId}`,{method:"DELETE"});
