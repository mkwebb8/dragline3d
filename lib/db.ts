...
export type Order = { id:string;square_payment_link_id?:string;square_payment_id?:string;customer_name:string;customer_email:string;address?:string;city?:string;state?:string;zip?:string;shipping_service?:string;shipping_cost?:number;subtotal?:number;setup_fee?:number;handling_fee?:number;handling_units?:number;total:number;square_fee?:number;refunded_amount?:number;status:OrderStatus;tracking_number?:string;notes?:string;square_invoice_url?:string;created_at?:string;updated_at?:string;order_items?:OrderItem[];inventory_consumed?:boolean };
